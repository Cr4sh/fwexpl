...
BOOLEAN SetObjectSecurityWorld(HANDLE hObject, ACCESS_MASK AccessMask)
{
    ...
    PSECURITY_DESCRIPTOR Descr = (PSECURITY_DESCRIPTOR)M_ALLOC(SdLength);
    if (Descr)
    {
        ...
        if (NT_SUCCESS(ns)) 
        {
            ...
            { 
                ...
                if (RtlValidSid(&Sid)) 
                {
                    ...
                    // allocate new DACL
                    PACL NewDacl = (PACL)M_ALLOC(NewDaclSize);
                    if (NewDacl)
                    {
                        // copy current DACL
...
BOOLEAN GetNormalizedModulePath(PANSI_STRING asPath, PANSI_STRING asNormalizedPath)
{
    ...
    {        
        ...
        ULONG FullPathLen = asPath->Length - Ptr + strlen(lpszLetter) + 1;
    ...
    NTSTATUS ns = RtlAnsiStringToUnicodeString(&usPath, &asFullPath, TRUE);
    if (NT_SUCCESS(ns))
    {
    ...
    if (!bRet)
    {
        ...
        for (ULONG i = 0; i < asFullPath.Length; i++)
        {
...
BOOLEAN RegSetValueKey(HANDLE hKey, PWSTR lpwcName, ULONG Type, PVOID Data, ULONG DataSize)
{
    ...
    NTSTATUS ns = ZwSetValueKey(hKey, &usValueName, NULL, Type, Data, DataSize);
    if (!NT_SUCCESS(ns))
    {
